...
class IntentClassifier:
    ...
    def _load_bert_model(self, model_path):
        ...
        model_state = torch.load(
            os.path.join(model_path, "model.pt"), map_location=self.device
        )
    ...
    def predict(self, text):
        """Classify intent from input text using loaded model.
        ...
        Args:
            ...
            Raises:
                Exception: If model not ready or prediction fails"""
        if not self.ready:
        ...
        top_probs, top_indices = torch.topk(
            probs, k=min(3, len(self.label_encoder.classes_))
        )

        results = []
        for prob, idx in zip(top_probs.cpu().numpy(), top_indices.cpu().numpy()):
            results.append(
                {"label": self.label_encoder.classes_[idx], "confidence": float(prob)}
            )
