...
class EvaluationVisualizer:
    ...
    def plot_confidence_distribution(self, save_path=None, figsize=(10, 6)):
        ...
        correct_conf = [conf for conf, pred, true in zip(confidences, predictions, true_labels)
                        if pred == true]
        incorrect_conf = [conf for conf, pred, true in zip(confidences, predictions, true_labels)
                          if pred != true]

        plt.figure(figsize=figsize)

        plt.hist(correct_conf, bins=50, alpha=0.7, label='Correct Predictions', color='green')
        plt.hist(incorrect_conf, bins=50, alpha=0.7, label='Incorrect Predictions', color='red')
        ...
        # Add statistics
        plt.axvline(np.mean(correct_conf), color='green', linestyle='--', alpha=0.8,
                    label=f'Correct Mean: {np.mean(correct_conf):.3f}')
        plt.axvline(np.mean(incorrect_conf), color='red', linestyle='--', alpha=0.8,
    ...
    def plot_class_performance(self, save_path=None, figsize=(12, 8)):
        ...
        sorted_data = sorted(zip(classes, precision, recall, f1_score, support),
        ...
        for i, sup in enumerate(support):
            plt.text(i, 0.05, f'n={sup}', ha='center', va='bottom', fontsize=8, rotation=90)
